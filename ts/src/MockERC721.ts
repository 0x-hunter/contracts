...
export type ContractContext = Web3ContractContext<
  MockERC721,
  MockERC721MethodNames,
  MockERC721EventsContext,
  MockERC721Events
>;
export type MockERC721Events = 'Approval' | 'ApprovalForAll' | 'Transfer';
export interface MockERC721EventsContext {
  Approval(
    parameters: {
      filter?: {
        owner?: string | string[];
...
export type MockERC721MethodNames =
  | 'new'
  | 'approve'
  | 'balanceOf'
  | 'getApproved'
...
export interface MockERC721 {
   ...
   * StateMutability: nonpayable
   * Type: function
   * @param _usr Type: address, Indexed: false
   * @param _tokenId Type: uint256, Indexed: false
   */
  mint(_usr: string, _tokenId: string): MethodReturnContext;
  /**
   * Payable: false
   * Constant: true
   * StateMutability: view
